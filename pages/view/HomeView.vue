<template>
	<view class="container pb-24">
		...
		<view class="grid-box">
			<view class="grid-item" @tap="navigateTo('/pages/map/map')">
			...
			<view class="grid-item disabled">
				...
				<text class="grid-text">住宿预订</text>
			</view>
			<view class="grid-item disabled">
				<view class="icon-wrap bg-purple"><text class="iconfont">📑</text></view>
				<text class="grid-text">政策帮办</text>
			</view>
			<view class="grid-item" @tap="navigateTo('/pages/alliance/alliance')">
...
const handleApply = (platform) => {
	uni.showModal({
		title: '接单确认',
		content: `申请已提交，数据将同步至${platform}，请等待审核。`,
		showCancel: false
	});
};
...
.id-card {
	...
	.card-no { font-size: 32rpx; font-family: 'Courier New', Courier, monospace; letter-spacing: 4rpx; }
...
.task-section {
	...
	.section-title {
		...
		gap: 10rpx;
		margin-bottom: 30rpx;
		.bolt-icon { color: #fbbf24; }
		.title-text { font-weight: bold; font-size: 32rpx; }
	}
}
...
.task-card {
	...
	.task-header {
		...
		justify-content: space-between;
		margin-bottom: 20rpx;
		.task-name { font-size: 28rpx; font-weight: bold; width: 80%; }
		.task-price { color: #ef4444; font-weight: bold; }
	}
	.task-footer {
		display: flex;
		justify-content: space-between;
